    ...
    Instance: function () {
      ...
      this.events = {
        ...
        onRegisterSubmit: async () => {
          const store = await this.ccm.store( { url: 'https://ccm2.inf.h-brs.de', name: 'dms-user' } );
          const { name, username, password } = $.formData( element.querySelector( '#register_form' ) );
          try {
            await store.set( {
              key: username,
              user: username,
              name,
              realm: 'cloud',
              token: this.user.hash.md5( password ),
              _: {
                creator: username,
                realm: 'cloud',
                access: {
            ...
            alert(this.lang.translate('register_success'));
            $.params( { view: 'home' }, true, true );
            await this.refresh();
          } catch (e) {
            alert(this.lang.translate('register_failed'));
          }
        },
        onLogin: async () => {
      ...
      this.render = {
        ...
        show: async app_key => {
          ...
          await this.handover.start( {
            ...
            tool: tool_meta.path,
            url: ( component, config ) => this.url + '?app=' + config[ 2 ].toString()
          } );
        },
        register: async () => {
          this.render.header();
          this.html.render( this.html.register(), element );
          this.lang && this.lang.translate();
        }
      };
